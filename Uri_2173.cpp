...
// (uses union by rank)
void Union2(struct subset subsets[], long int x, long int y)
{
...
long int KruskalMST2(struct Graph* graph)
{
    ...
    // array of edges
    qsort(graph->edge, graph->E, sizeof(graph->edge[0]), myComp2);
...
int main() {
    ...
    long int soma_minima1; // Sum of tree weights
    long int soma_minima2;
    
    scanf("%ld%ld", &V, &E);
    
    while(V != 0 && E != 0){
    
        soma_minima1 = soma_minima2 = 0;
        ...
        soma_minima1 = KruskalMST(graph);
        soma_minima2 = KruskalMST2(graph);
        
        printf("%ld\n", soma_minima2 - soma_minima1);
